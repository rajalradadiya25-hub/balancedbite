...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = "SELECT * FROM `$dbname`.`user_info` WHERE id='$id'";
...
if (isset($_POST['login'])) 
{
    ...
    // table name check karo — agar aapka table 'users' hai to aise hi rakho
    ...
    if ($row = mysqli_fetch_assoc($result)) 
    {
        if (password_verify($password, $row['password']) || $password == $row['password']) 
        {
            ...
            // User ke liye role ka use hi nahi karna
...
if (isset($_SESSION['selected_plan'])) {
    ...
    if (isset($plan['plan'], $plan['amount'], $plan['discount'], $plan['final'])) {
        unset($_SESSION['selected_plan']);
        header("Location: api/payment.php?plan={$plan['plan']}&amount={$plan['amount']}&discount={$plan['discount']}&final={$plan['final']}");
        exit;
    }
}
?>
                    ...
                    <div class="row mt-2">
                        <div class="col-md-4 offset-md-4">
                            <div class="mb-3">
                                ...
                                <p>Don't have an account? <a href="signup.php" style="color: rgb(0,128,0);">Register</a></p>
                            </div>
